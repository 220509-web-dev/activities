...
# -z string: True if the length of string is zero.
  ...
  echo "Example: $0 SJones@cleanup.com"
  exit 1
fi
...
query="select * from tryagain4.app_users where email = '$1';"
...
# echo "select * from tryagain4.app_users where email = '$1';" | docker exec -i Sigg-db psql -U postgres
    # ^^^^ This will return a blank table
        #   id | first_name | last_name | email | username | password 
        #   ----+------------+-----------+-------+----------+----------
        #   (0 rows)

# echo "select * from tryagain4.app_users where email = 'SJones@cleanup.com';" | docker exec -i Sigg-db psql -U postgres
    # ^^^^ This will return user info associated with said email
        #    id | first_name | last_name |       email        | username | password 
        #   ----+------------+-----------+--------------------+----------+----------
        #     8 | Sara       | Jones     | SJones@cleanup.com | SJones   | Qwerty12
        #   (1 row)

# echo "select * from tryagain4.app_users where email = 'SJones@cleanup.com';" | docker exec -i Sigg-db psql -U postgres | grep 'SJones@cleanup.com'
    # ^^^^ This will clean it up even more
        #   8 | Sara       | Jones     | SJones@cleanup.com | SJones   | Qwerty12

# echo "select * from tryagain4.app_users where email = 'SJones@cleanup.com';" | docker exec -i Sigg-db psql -U postgres | grep 'SJones@cleanup.com' | wc -l
